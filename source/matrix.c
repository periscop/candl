 ...
 */
CandlMatrix * candl_matrix_dependence(source, target, array_s, array_t,
                                      ref_s, ref_t, depth, before, nb_par)
CandlMatrix  * source, * target, * array_s, * array_t ;
...
{ int i, j, nb_rows, nb_columns, nb_dimensions, constraint, s_dims, t_dims ;
  ...
  s_dims = source->NbColumns - nb_par - 2 ;
  t_dims = target->NbColumns - nb_par - 2 ;
   ...
   */
  nb_rows = source->NbRows + target->NbRows + nb_dimensions + depth ;
   ...
   */
  int min_depth = s_dims < t_dims ? s_dims : t_dims;

  /* 1. The source iteration domain constraints. */
  constraint = 0 ;
  for (i=0;i<source->NbRows;i++)
  { for (j=0;j<=s_dims;j++)
    CANDL_assign(system->p[constraint][j],source->p[i][j]) ;

    for (j=s_dims+1;j<source->NbColumns;j++)
    CANDL_assign(system->p[constraint][j+t_dims],source->p[i][j]) ;

    constraint++ ;
  }

  /* 2. The target iteration domain constraints. */
  for (i = 0; i < target->NbRows; i++)
  { CANDL_assign(system->p[constraint][0],target->p[i][0]) ;

    for (j = 1; j < target->NbColumns; j++)
    CANDL_assign(system->p[constraint][j + s_dims], target->p[i][j]) ;
