...
{ CandlOptions * options ;
  ...
  options->violgraph = 1 ; /* Do print the violation graph.*/
...
{ printf("Candl %s %s bits   The Chunky Dependence Analyzer\n",
  ...
  printf(
  "-----\n"
  "This is a dependence analyzer for static control programs, coming from the\n"
  "CHUNKY project: a research tool for data-locality improvement. This \n"
 ...
 */
void candl_options_set(int * option, int argv, char ** argc, int * number)
{ char ** endptr ;
 ...
 */
void candl_options_read(argv, argc, input, output, options)
int argv ;
char ** argc ;
FILE ** input, ** output ;
CandlOptions ** options ;
{ int i, infos=0, input_is_set=0 ;
  ...
  *output = stdout ;
  ...
  for (i=1;i<argv;i++)
  if (argc[i][0] == '-')
  { if (strcmp(argc[i],"-waw")   == 0)
    candl_options_set(&(*options)->waw,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-raw")   == 0)
    candl_options_set(&(*options)->raw,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-war")   == 0)
    candl_options_set(&(*options)->war,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-rar")   == 0)
    candl_options_set(&(*options)->rar,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-commute")   == 0)
    candl_options_set(&(*options)->commute,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-fullcheck")   == 0)
    candl_options_set(&(*options)->fullcheck,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-depgraph")   == 0)
    candl_options_set(&(*options)->depgraph,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-violgraph")   == 0)
    candl_options_set(&(*options)->violgraph,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-scalpriv")   == 0)
    candl_options_set(&(*options)->scalar_privatization,argv,argc,&i) ;
    else
    if (strcmp(argc[i],"-view")   == 0)
    (*options)->view = 1 ;
    else
    ...
    { if (i+1 >= argv)
      { fprintf(stderr, "[Candl]ERROR: no output name for -o option.\n") ;
        exit(1) ;
      }
      ...
      *output = stdout ;
      ...
      { *output = fopen(argc[i+1],"w") ;
        if (*output == NULL)
        { fprintf(stderr, "[Candl]ERROR: can't create output file %s.\n",
	          argc[i+1]) ;
          exit(1) ;
        }
      }
    ...
    { input_is_set = 1 ;
      ...
      *input = stdin ;
      ...
      { *input = fopen(argc[i],"r") ;
        if (*input == NULL)
        { fprintf(stderr, "[Candl]ERROR: %s file does not exist.\n",argc[i]) ;
          exit(1) ;
        }
      }
    } 
